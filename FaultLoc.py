...
v1 = pdf.ShowPdf()
v2 = v1.pdf_view(tab3,
                 pdf_location=r"D:\Desktop\Uninorte\Onceavo Semestre\PF - Ing Electrica\PF Codigo GUI\fault-location-pf-master\Manual-de-Usuario-PFLOC.pdf",
                 width=80, height=100)
v2.pack()
...
def FaultLoc():
    ...
    try :
        ...
        ms.showerror("Error", "Ingreso erroneo de los datos")    
...
img = Image.open(r'D:\Desktop\Uninorte\Onceavo Semestre\PF - Ing Electrica\PF Codigo GUI\fault-location-pf-master\logo-un.jpg')

img = img.resize((150,45), Image.ANTIALIAS)

img = ImageTk.PhotoImage(img)

label = tkinter.Label(tab1, image = img).place(x=70, y=5)
label = tkinter.Label(tab2, image = img).place(x=70, y=5)
...
def Graph2():
    ...
    plot3.plot(vector_distancia*line_distance/100, error_array, 'r')
    plot3.legend(labels=['Validación de fallas'],loc='lower left')
    plot3.title.set_text("Validación de fallas")
    plot3.set_xlabel('Distancia de la falla (km)')
...
buttongraph2 = tkinter.Button(tab2, text='Realizar Validación', command=lambda:[VarReading(),VarReading2(),FaultLoc2(),Graph2()])
